 ...
 *    settings rows.
 * 2. get_file_map() — runtime id => file map DERIVED from DEFINITIONS
 *    (legacy-keyed until the slug migration flips runtime ids). get_all()
...
class Module_Registry {
    ...
    /**
     * Merge schema defaults into a (possibly sparse or filtered)
     * definition. Filtered definitions MUST be normalized before
     * validation so optional fields carry sane values.
     */
    public static function normalize( array $def ): array {
        return array_merge( self::DEFAULTS, $def );
    }
    ...
    /**
     * LEGACY alias for the derived runtime id => file map.
     *
     * The hand-written id => file array was removed once DEFINITIONS
     * became the source of truth; existing callers (loader boot, setup
     * wizard) receive an identical map via get_file_map().
     *
     * @deprecated Use get_definitions() for metadata or get_file_map()
     *             for the loader map.
     */
    public static function get_all(): array {
        return self::get_file_map();
    }
}
