...
| 2026-04-10 | smart-menu-organizer + admin (sidebar) | **DEFERRED** — Smart Menu Organizer (JS, runs on the page) creates its own set of section headers (Content / Build / Manage / Other) and reorders WP menu items into them, while `Admin::inject_section_labels()` (PHP, runs on `admin_menu`) creates a different set of canonical section separators (CONTENT / SECURITY / DESIGN / TOOLS / CONFIGURE). When both run together with all 85 modules active, the sidebar shows duplicate "CONTENT" headers, Dashboard ends up under the wrong section, WPTransformed lands under DESIGN instead of inside one of Smart Menu Organizer's groups, and the menu hierarchy becomes visually muddled. The two systems compete for ownership of menu organization. Per `docs/ui-restructure-spec.md` Section 3, Smart Menu Organizer is supposed to slot items into the canonical groups, not invent its own. **Fix direction**: align both systems on a single source of truth — either Smart Menu Organizer reads its sections from `Admin::inject_section_labels()` (or a shared registry), or the PHP injector defers entirely when Smart Menu Organizer is active. Requires touching both PHP and JS sides; warrants its own scoped fix later. | Open |
